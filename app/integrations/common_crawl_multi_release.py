...
adds a multi-release union over complete graph files that are already present
locally. It does not download large graph releases at audit time.
"""
...
def collect_all_local_releases(url: str) -> dict:
    ...
    if not domain:
        return {**base, "status": "invalid_target", "releases_checked": [], "total_backlinks": 0, "referring_domains": 0, "backlinks": []}

    root = local_graph_dir()
    if root is None:
        return {**base, "status": "not_configured", "releases_checked": [], "total_backlinks": 0, "referring_domains": 0, "backlinks": [], "message": "Local Common Crawl graph directory was not found."}

    releases = available_local_releases(root)
    if not releases:
        return {**base, "status": "graph_missing", "releases_checked": [], "total_backlinks": 0, "referring_domains": 0, "backlinks": [], "message": f"No complete local Common Crawl domain graph releases were found in {root}."}
    ...
    for release in releases:
        ...
        stat = {"graph_release": release, "status": "ok", "target_node_id": None, "incoming_node_ids": 0, "resolved_referring_domains": 0}
        ...
        try:
            ...
            for source_domain in domains.values():
    ...
    backlinks = sorted(by_domain.values(), key=lambda item: item["referring_domain"])
