...
def analyze(data):
    ...
    for l in links:
        if l.get("internal") or l.get("target_url", "").startswith(data.get("start_url", "")):
            incoming[l.get("target_url", "")] += 1
            outgoing[l.get("source_url", "")] += 1
    ...
    for p in pages:
        ...
        links_out = outgoing[url]

        check(url, 200 <= status < 400, "critical" if status >= 500 else "high", f"HTTP {status}")
        check(url, bool(title), "critical", "Missing title")
        if title: check(url, 30 <= len(title) <= 60, "medium" if len(title)>60 else "notice", "Long title" if len(title)>60 else "Short title", f"{len(title)} characters")
        check(url, bool(desc), "medium", "Missing meta description")
        if desc: check(url, 70 <= len(desc) <= 160, "notice", "Long meta description" if len(desc)>160 else "Short meta description", f"{len(desc)} characters")
    ...
    # Duplicate metadata is a site-level/page-level issue.
    for field, label in (("title", "Duplicate title"), ("meta_description", "Duplicate meta description")):
        groups=defaultdict(list)
        for p in pages:
            v=(p.get(field) or "").strip().lower()
            if v: groups[v].append(p.get("url"))
        for urls in groups.values():
            if len(urls)>1:
                for u in urls: findings[u].append(("medium", label, f"{len(urls)} pages")); counts["medium"] += 1

    # Score each page using weighted checks. Failed checks reduce the page from a 50% floor,
    # while passed checks retain their contribution. The floor prevents a broken site from becoming negative.
    ...
    # Site health deliberately focuses on high-impact site-wide infrastructure.
    health_penalty=0.0
    ...
    if status_fail: health_penalty += min(35, status_fail/max(len(pages),1)*35)
    if noindex: health_penalty += min(25, noindex/max(len(pages),1)*25)
    ssl=(site.get("domain") or {}).get("ssl") or {}
    if ssl.get("enabled") is False: health_penalty += 25
    if not site.get("domain",{}).get("hostname"): health_penalty += 10
    if not site.get("platform"): health_penalty += 5
    ...
    for p in pages:
        url=p.get("url", "")
        fs=findings.get(url, [])
        page_rows.append({"url":url,"issues":[{"severity":s,"message":m,"detail":d} for s,m,d in fs],"score":round(page_scores[pages.index(p)]) if page_scores else 0})
    return {"summary":{"seo_score":seo_score,"site_health":site_health,"pages_crawled":len(pages),"pages_discovered":data.get("site",{}).get("pages_discovered",len(pages)),"critical":counts["critical"],"high":counts["high"],"medium":counts["medium"],"notice":counts["notice"],"total_issues":sum(counts.values()),"checks":total_checks,"passed":passed},"site":site,"pages":page_rows}
...
def html_report(audit_data, crawl_data):
    ...
    for p in audit_data["pages"]:
        ...
        for i in p["issues"]:
            text=i["message"]
            if i.get("detail"): text += f" ({i['detail']})"
...
def main():
    ...
    path=Path(sys.argv[1]); data=load_crawl(path); html=generate_report(data,path); print(f"Report: {html}")
